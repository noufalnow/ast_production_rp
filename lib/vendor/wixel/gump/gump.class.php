...
class GUMP
{
     ...
     */
    public function add_custom_error($field,$cMsgs=array())
    {
    	foreach($cMsgs as $valids=>$message){
	    	$validator = $valids;
	    	$this->custom_errors[$field][$validator]=$message;
    	}
    ...
    public function get_errors_array()
    {
        ...
        foreach ($this->errors as $error) { 
            ...
            //Noufal s
            if ($message = $this->custom_errors[$error['field']][$error['rule']]) {
     ...
     * @return string
     */
    protected function FILTER_UNSAFE_RAW($value, array $params = [])
    {
     ...
     * @return bool
     */
    protected function validate_required($field=null, array $input = [], array $params = [], $value=null)
    {
    ...
    protected function validate_date($field, array $input, array $params = [], $value)
    {
        ...
        /** new methode added by Noufal 18/11/2018 **/
        list($dd,$mm,$yyyy) = explode('/',$input[$field]);
    ...
    protected function validate_file_size($field, array $input, array $params = [], $value)
    {
        return is_numeric($value) && is_numeric($params[0]) && ($value <= $params[0]);
    }
    ...
    protected function validate_extension($field, $input, array $params, $value)
    {
        if (is_array($input[$field]) && $input[$field]['error'] === 0) {
            ...
            //$path_info = pathinfo($input['name']); //modified by Noufal $input[$field]['name'] => $input['name']
     ...
     * @return mixed
     */
    protected function validate_invalid($field, array $input, array $params = [], $value)
    {
