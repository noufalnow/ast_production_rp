...
class masterController extends mvc
{
    ...
    public function empcontractAction()
    {
        ...
        $form->addElement('project', 'Project ', 'text', '');
        ...
        if ($_POST) {
            if (! isset($_SERVER['HTTP_X_REQUESTED_WITH']) and strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
                ...
                if ($valid == true) {
                    ...
                    $data = array(
                        'emc_status' => $valid['status'],
                        'emc_emp_id' => $decRefId,
                        'emc_vhl_id' => $valid['vehicle'],
                        'emc_cust_id' => $valid['customer'],
                        'emc_project' => $valid['project'],
                        'emc_location' => $valid['location'],
                        'emc_date_start' => $dtWef,
                        'emc_note' => $valid['note']
                    );
                    $insert = $contractObj->add($data);
                    if ($insert) {

                        $feedback = ' Contract Updated successfully';
    ...
    public function empcontracteditAction()
    {
        ...
        $conDet = $contractObj->getDetById($decRefId);
        ...
        if ($_POST) {
            if (! isset($_SERVER['HTTP_X_REQUESTED_WITH']) and strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) != 'xmlhttprequest') {
                ...
                if ($valid == true) {
                    ...
                    $update = $contractObj->modify($data, $decRefId);
                    if ($update) {
            ...
            $doe = DateTime::createFromFormat(DFS_DB, $conDet['emc_date_start']);
            $doe = $doe->format(DF_DD);

            $form->startDt->setValue($doe);
            $form->vehicle->setValue($conDet['emc_vhl_id']);
            $form->customer->setValue($conDet['emc_cust_id']);
            $form->project->setValue($conDet['emc_project']);
            $form->location->setValue($conDet['emc_location']);
            $form->note->setValue($conDet['emc_note']);
        }
    ...
    public function empcontractupdateAction()
    {
        ...
        if ($_POST) {
            ...
            if (! empty($conDet['emc_date_end'])) {
                $doe = DateTime::createFromFormat(DFS_DB, $conDet['emc_date_end']);
                $doe = $doe->format(DF_DD);
            }

            $form->status->setValue($conDet['emc_status']);
    ...
    public function profileAction()
    {
        ...
        require_once __DIR__ . '/../admin/!model/empcontract.php';
        $empcontractObj = new empcontract();
        $this->view->empContract = $empcontractObj->getDetByEmployeeId(array(
            'emc_emp_id' => $employeeDetail['emp_id']
        ));
